...
DOCKER_BUILDKIT=1 docker build --platform linux/amd64 --pull=false -t oneauth/backend:v1.0.9 -f release/Dockerfile.backend.offline .
DOCKER_BUILDKIT=1 docker build --platform linux/amd64 --pull=false -t oneauth/gateway:v1.0.9 -f release/Dockerfile.gateway.offline .

echo "Saving images..."
docker save oneauth/backend:v1.0.9 -o "$PKG_DIR/images/backend-v1.0.9.tar"
docker save oneauth/gateway:v1.0.9 -o "$PKG_DIR/images/gateway-v1.0.9.tar"
